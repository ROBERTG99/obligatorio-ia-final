...
import json
import pickle
# Nuevas importaciones para paralelización total
import psutil
...
# ---------------------------------------------------------------------------
# Función auxiliar para entrenar y evaluar una combinación de hiperparámetros
# ---------------------------------------------------------------------------

def _train_and_evaluate_combo(params: Dict[str, Any]) -> Dict[str, Any]:
    """Entrena y evalúa un agente con los parámetros dados.

    Retorna un diccionario con score promedio, desviación y mejor episodio."""

    # Silenciar salida del entorno para procesos hijos
    ...
    # Entrenamiento reducido para búsqueda (2 000 episodios)
    TRAIN_EPISODES = 2000
    ...
    # Evaluación rápida (250 episodios)
    EVAL_EPISODES = 250
...
def buscar_hiperparametros_optimos():
    ...
    # Grid equilibrado solicitado (9 combinaciones)
    param_grid = {
        'learning_rate': [0.03, 0.05, 0.07],  # α
        'discount_factor': [0.98, 0.99, 0.997],  # γ
        # Parámetros fijos según especificación
        'epsilon': [0.3],
        'epsilon_decay': [0.9996]  # Decaimiento para ~40 k pasos
    }
    
    print("📊 Grid equilibrado:")
    for param, values in param_grid.items():
        print(f"   • {param}: {values}")
    
    import itertools
    combinaciones = [dict(zip(param_grid.keys(), combo))
                    for combo in itertools.product(*param_grid.values())]

    print(f"   • Combinaciones: {len(combinaciones)} (ejecución paralela)")
    ...
    print(f"⚡ Utilizando {num_cores} cores lógicos para Grid Search")
    ...
    with ProcessPoolExecutor(max_workers=num_cores) as executor:
        future_to_params = {
            executor.submit(_train_and_evaluate_combo, params): params for params in combinaciones
        }
        ...
        for i, future in enumerate(as_completed(future_to_params), 1):
            try:
                ...
                print(f"   • [{i}/{len(combinaciones)}] Params: {result['params']} -> "
                      f"Score: {result['score']:.2f} (Std {result['std']:.2f}, "
                      f"Best {result['best']:.2f})")

                if result['score'] > best_score:
                    best_score = result['score']
                    best_params = result['params']
                    print("     🏆 Nuevo mejor hasta ahora!")

            except Exception as e:
                print(f"❌ Error evaluando {future_to_params[future]}: {e}")
